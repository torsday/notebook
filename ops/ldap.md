...
Relative Distinguished Name (RDN) | Attributes *unique at its level in the hierarchy*. RDNs may be multi-valued, combining two or more attributes using `+` e.g. cn+uid. The term RDN is only meaningful when used as part of a DN to uniquely describe the attributes on the path UP the DIT from a selected entry (or search start location) to the directory root (or more correctly the Root DSE).
